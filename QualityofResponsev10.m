function QualityofResponsev10(Ridges,Baselines)
...
for(i=3:length(file))
    ...
    if(file(i).isdir)
        ...
        for(i=1:m)
            ...
            AngleFishFinal(i,s)=Angle(i,x(1)+RelAngRxn+buffer-1);
...
if(~isempty(Ridges))%Pool the results from the rigde experiments and baseline experiments
    ...
    RidgeFAF=AngleFishFinal(:,Ridges);
    BaselineFAF=AngleFishFinal(:,Baselines);
    RidgeFAF=RidgeFAF(:);BaselineFAF=BaselineFAF(:);
    RidgeFAF=RidgeFAF(RidgeZN);BaselineFAF=BaselineFAF(BaselineZN);
    
    RidgeFAI=AngleFishInitial(:,Ridges);
    BaselineFAI=AngleFishInitial(:,Baselines);
...
%     RidgeAB=RidgeAB(DBR);BaselineAB=BaselineAB(DBB);
    ...
    dlmwrite('RidgeInitvsEstV10.txt','Angles - Ridge','delimiter','')
    dlmwrite('RidgeInitvsEstV10.txt','Initial - Estimate','delimiter','','-append')
    dlmwrite('RidgeInitvsEstV10.txt',[RidgeAI RidgeAE],'delimiter','\t','-append')
    
    dlmwrite('BaselineInitvsEstV10.txt','Angles - Baseline','delimiter','')
    dlmwrite('BaselineInitvsEstV10.txt','Initial - Estimate','delimiter','','-append')
    dlmwrite('BaselineInitvsEstV10.txt',[BaselineAI BaselineAE],'delimiter','\t','-append')
    
    dlmwrite('RidgeAllV10.txt','Ridge - Angle - Distance','delimiter','')
    dlmwrite('RidgeAllV10.txt','Initial - Reaction - Estimate - Best - Fish Angle Initial - Fish Angle Final','delimiter','','-append')
    dlmwrite('RidgeAllV10.txt',[RidgeAI RidgeAR RidgeAE RidgeAB RidgeDI RidgeDR RidgeDE RidgeDB RidgeFAI RidgeFAF],'delimiter','\t','-append')
    
    dlmwrite('BaselineAllV10.txt','Baseline - Angle - Distance','delimiter','')
    dlmwrite('BaselineAllV10.txt','Initial - Reaction - Estimate - Best - Fish Angle Initial - Fish Angle Final','delimiter','','-append')
    dlmwrite('BaselineAllV10.txt',[BaselineAI BaselineAR BaselineAE BaselineAB BaselineDI BaselineDR BaselineDE BaselineDB BaselineFAI BaselineFAF],'delimiter','\t','-append')

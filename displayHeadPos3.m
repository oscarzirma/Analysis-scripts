function displayHeadPos(T,X ,Y, Z, Yaw, Rol, Pit, Tm, Xm, Ym, Zm, Yawm, Rolm, Pitm)%,stimulus_position,stimuli_positions,fixCenterX,fixCenterY)
...
for i=1:l %plot each dimension
    subplot(131)
    title('Y')
    plot(cell2mat(T(i)),cell2mat(Y(i)));
    subplot(132)
    title('x')
    a=cell2mat(T(i));
    plot(a(1:end-1),diff(cell2mat(X(i))),'g');
    subplot(133)
    title('Z')

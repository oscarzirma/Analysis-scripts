function [Peaki,Peako]=multiMicroWithPeaksFilter(Data,NumCycles,TaperCycles,Delay,freq,amp,SRate,Filter)
...
%If display is 'on' it will display each iteration. Will also filter out
%excess peaks, averaging them into a single peak.
...
for(j=1:(Numamp))
    for(k=1:Numfreq)
        ...
        cycles=NumCycles-2*TaperCycles;
        [Fi,Ai,Fo,Ao,Pi,Po,I]=FMicrophonicFilter(Data,[s fe],SRate,frq,cycles);

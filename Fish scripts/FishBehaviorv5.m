function [PelletIndices,RelDis,RelAng,Angle,NetDis]=FishBehaviorv5(position,pellet,fieldwidthpix,fieldwidthcm,fps,numframes)
...
for(i=1:length(PI))
    ...
    Fi=position(k,1:2);%fish initial position
    for(j=1:numframes)
        ...
        Dis(k)=sqrt(sum((Pf-Fi).^2))/c;%distance between fish current and fish initial position
    ...
    if(d>size(RelPos,1))
        ...
        Ang(1:length(AF)-PI(i))=AF(PI(i):end-1);
        NDis(1:length(Dis)-PI(i))=Dis(PI(i):end-1);

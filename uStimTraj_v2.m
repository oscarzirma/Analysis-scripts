function [uX uY uZ uE1 uE2] = uStimTraj_v2(headPosition,microstim_index,STI,EYE,filename)
...
for i=1:length(STI)%get microstim and eye traces
    x=cell2mat(STI(i));y=cell2mat(EYE(i));
    if ~isempty(x) M(i,:)=x(2,:);end
    if ~isempty(y) E1(i,:)=y(2,:);E2(i,:)=y(3,:);end
end
te=x(1,:);
...
uE1(:,1) = nanmean(E1(:,bx-5:bx+5),2);
uE1(:,2) = nanmean(E1(:,mx-5:mx+5),2);
uE1(:,3) = uE1(:,2)-uE1(:,1);

uE2(:,1) = nanmean(E2(:,bx-5:bx+5),2);
uE2(:,2) = nanmean(E2(:,mx-5:mx+5),2);
uE2(:,3) = uE2(:,2)-uE2(:,1);

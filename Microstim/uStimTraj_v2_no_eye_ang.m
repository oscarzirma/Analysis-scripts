function [uY uR uP ms_amp] = uStimTraj_v2_no_eye_ang(headPosition,microstim_index,STI,filename)
...
[t tmp1 tmp2 tmp3 Ydist Rdist Pdist] = getHeadPosfromCells_ang(headPosition,0,3,.008);
Ydist=-Ydist;Pdist=-Pdist;
n=size(Ydist,1); %M=zeros(n,2.4e4);E=M;
...
uR(:,1) = nanmean(Rdist(:,bx-5:bx+5),2);
uR(:,2) = nanmean(Rdist(:,mx-5:mx+5),2);
uR(:,3) = uR(:,2)-uR(:,1);


uP(:,1) = nanmean(Pdist(:,bx-5:bx+5),2);
uP(:,2) = nanmean(Pdist(:,mx-5:mx+5),2);
uP(:,3) = uP(:,2)-uP(:,1);
...
subplot(2,2,[1 3])
scatter3(uY(:,1),uR(:,1),uP(:,1),'k')
hold on
scatter3(uY(~m,2),uR(~m,2),uP(~m,2),'b')
scatter3(uY(m,2),uR(m,2),uP(m,2),'r')
...
subplot(2,2,2)
scatter(uY(:,1),uR(:,1),'k');hold on;scatter(uY(~m,2),uR(~m,2),'b');scatter(uY(m,2),uR(m,2),'r');xlabel('Yaw');ylabel('Roll');axis equal
subplot(2,2,4)
scatter(uY(:,1),uP(:,1),'k');hold on;scatter(uY(~m,2),uP(~m,2),'b');scatter(uY(m,2),uP(m,2),'r');xlabel('Yaw');ylabel('Pitch');axis equal


for i=1:n%plot lines connecting baseline and stimmed points from the same trial
        subplot(2,2,[1 3])
        line([uY(i,1) uY(i,2)],[uR(i,1) uR(i,2)],[uP(i,1) uP(i,2)],'Color',j(ms_amp_scaled(i),:))
        subplot(2,2,2); line([uY(i,1) uY(i,2)],[uR(i,1) uR(i,2)],'Color',j(ms_amp_scaled(i),:))
        subplot(2,2,4); line([uY(i,1) uY(i,2)],[uP(i,1) uP(i,2)],'Color',j(ms_amp_scaled(i),:))
...
for i=1:n%plot trajectory w/ color indicating stim strength
    subplot(2,2,[1 3])
    line([0 uY(i,3)],[0 uR(i,3)],[0 uP(i,3)],'Color',j(ms_amp_scaled(i),:));
        
    subplot(2,2,2)
    line([0 uY(i,3)],[0 uR(i,3)],'Color',j(ms_amp_scaled(i),:));
    
        subplot(2,2,4)
    line([0 uY(i,3)],[0 uP(i,3)],'Color',j(ms_amp_scaled(i),:));


%     if(m(i)) 
%         line([0 uY(i,3)],[0 uR(i,3)],[0 uP(i,3)],'Color','r'); 
%     else
%         line([0 uY(i,3)],[0 uR(i,3)],[0 uP(i,3)],'Color','b');
%     end
end
...
subplot(2,2,[1 3])
line([0 nanmean( uY(m,3))],[0 nanmean( uR(m,3))],[0 nanmean( uP(m,3))],'Color','r')
line([0 nanmean( uY(~m,3))],[0 nanmean( uR(~m,3))],[0 nanmean( uP(~m,3))],'Color','b')
xlabel('Yaw');ylabel('Roll');zlabel('Pitch');
view(3)

subplot(2,2,2)
line([0 nanmean( uY(m,3))],[0 nanmean( uR(m,3))],'Color','r','LineWidth',3)
line([0 nanmean( uY(m,3))+nanstd( uY(m,3))],[0 nanmean( uR(m,3))+nanstd( uR(m,3))],'Color','r')
line([0 nanmean( uY(m,3))-nanstd( uY(m,3))],[0 nanmean( uR(m,3))-nanstd( uR(m,3))],'Color','r')
line([0 nanmean( uY(~m,3))],[0 nanmean( uR(~m,3))],'Color','b','LineWidth',3)
line([0 nanmean( uY(~m,3))+nanstd( uY(~m,3))],[0 nanmean( uR(~m,3))+nanstd( uR(~m,3))],'Color','b')
line([0 nanmean( uY(~m,3))-nanstd( uY(~m,3))],[0 nanmean( uR(~m,3))-nanstd( uR(~m,3))],'Color','b')
xlabel('Yaw');ylabel('Roll');axis equal

subplot(2,2,4)
line([0 nanmean( uY(m,3))],[0 nanmean( uP(m,3))],'Color','r','LineWidth',3)
line([0 nanmean( uY(m,3))+nanstd( uY(m,3))],[0 nanmean( uP(m,3))+nanstd( uP(m,3))],'Color','r')
line([0 nanmean( uY(m,3))-nanstd( uY(m,3))],[0 nanmean( uP(m,3))-nanstd( uP(m,3))],'Color','r')
line([0 nanmean( uY(~m,3))],[0 nanmean( uP(~m,3))],'Color','b','LineWidth',3)
line([0 nanmean( uY(~m,3))+nanstd( uY(~m,3))],[0 nanmean( uP(~m,3))+nanstd( uP(~m,3))],'Color','b')
line([0 nanmean( uY(~m,3))-nanstd( uY(~m,3))],[0 nanmean( uP(~m,3))-nanstd( uP(~m,3))],'Color','b')
...
subplot(131)
scatter(uY(:,1),uY(:,3),3.*ms_amp_scaled,ms_amp_scaled);xlabel('baseline Yaw');ylabel('microstim shift Yaw');

subplot(132)
scatter(uR(:,1),uR(:,3),3.*ms_amp_scaled,ms_amp_scaled);xlabel('baseline Roll');ylabel('microstim shift Roll');

subplot(133)
scatter(uP(:,1),uP(:,3),3.*ms_amp_scaled,ms_amp_scaled);xlabel('baseline Pitch');ylabel('microstim shift Pitch');
...
scatter(ms_amp.*10^3,sqrt(uY(:,3).^2 + uR(:,3).^2 + uP(:,3).^2),100)

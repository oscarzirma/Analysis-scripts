function Vid_Mag_Correlation_static(centroid,mdata,shift)
...
m1_vs_X = aligned_mag_vid_remove_movements(v_time,centroid(:,1),mdata(1,:)./1000,mdata(2,:),shift_opt);%find temporally aligned m1 vs x trace
...
m1_vs_Y = aligned_mag_vid_remove_movements(v_time,centroid(:,2),mdata(1,:)./1000,mdata(2,:),shift_opt);
m1ycor=corrcoef(m1_vs_Y);
m1yp =polyfit(m1_vs_Y(:,2),m1_vs_Y(:,1),1);

m2_vs_X = aligned_mag_vid_remove_movements(v_time,centroid(:,1),mdata(1,:)./1000,mdata(3,:),shift_opt);
m2xcor=corrcoef(m2_vs_X);
m2xp =polyfit(m2_vs_X(:,2),m2_vs_X(:,1),1);

m2_vs_Y = aligned_mag_vid_remove_movements(v_time,centroid(:,2),mdata(1,:)./1000,mdata(3,:),shift_opt);
...
figure
subplot(221)
plot(m2_vs_X(:,1));title('X')
subplot(222)
plot(m1_vs_Y(:,1));title('Y')
subplot(223)
plot(m2_vs_X(:,2));title('Mb')
subplot(224)
plot(m1_vs_Y(:,2));title('Ma')

% figure
% subplot(221)
% plot(v_time,m2_vs_X(:,2));title('X')
% subplot(222)
% plot(v_time,m1_vs_Y(:,2));title('Y')
% subplot(223)
% plot(v_time,m2_vs_X(:,1));title('Mb')
% subplot(224)
% plot(v_time,m1_vs_Y(:,1));title('Ma')

...
camera_distance = 14;%camera distance in inches
...
fulcrum_pupil_distance = 6.2;%cornea-pupil distance in mm
...
% subplot(321)
 pXAB(:,1)=atand((pXAB(:,1)-mX)./fulcrum_pupil_distance);
...
% 
% subplot(323)
 pYAB(:,1)=atand((pYAB(:,1)-mY)./fulcrum_pupil_distance);
...
subplot(211)
[Fpxab Gpxab]=fit(pXAB(:,2:3),pXAB(:,1),'poly11');
...
subplot(212)
[Fpyab Gpyab]=fit(pYAB(:,2:3),pYAB(:,1),'poly11');
hist(pYAB(:,1)-feval(Fpxab,pYAB(:,2:3)),20)
...
hold on
plot(Tm./1000,feval(Fpyab,[smooth(A,50) smooth(B,50)]),'k')

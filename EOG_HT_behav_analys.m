function [Events]=EOG_HT_behav_analys(eog,ht)
...
for k =1:n
    ...
    if(k==1)
        te=e(1,:);end%time index
    e1=e(2,:);
    e1f=loadLPFilter(e1,30,.5);%filter eog

%     subplot(211)
%     plot(e1f)
    
    e1fd=diff(e1f);ted=te(1:end-1);%differentiate eog and replace time index

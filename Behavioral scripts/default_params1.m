 notes    = 'stage_1';%any relevant notes, e.g. stim parameters


%%
%%Visual stimulus parameters 
stim_radius          = 4; %stimulus radius in degrees
stim_duration_a      = 50; %stimulus duration in milliseconds (ms). if array, will sample all elements of array equally 
stim_delay           = 300; %delay between initiation of subtrial and stimulus presentation in ms
stim_contrast        =[1 .25 .1];% %stimulus contrast. if array, will sample all elements of array equally 
...
gabor_frequency  = 1;%gabor frequency in cycles per degree
...
response_box_radius = 25;%response box radius in degrees
...
fixed_response_box = false; %if true, the response will be fixed in angular space (rather than following the stim location). response_box_loc defines the distance from (0,0) in [az el]
response_box_loc = [0 -30]; %response box location for single. also much change 'generate_response_boxes' to maintain fixed location
...
random_response_box_loc = true;%if true, response box will occur randomly at locations specified in response_box_array
response_box_array      = [-20 -30;0 -30;20 -30];%absolute location of response boxes in degree [x1 y1;x2 y2]
...
total_fixation_time  = [100];%total delay until trial initiation with stim display, reward chime, reward enabling, and cross removal on response trials. if equal to soft initiation time, hard initiations will be delayed by ~10 ms 
trial_initiation_delay = [25];%amount of time before the hard initiation that the recordings should be initiated. IF ARRAY, WILL RANDOMLY SELECT
...
%Gaze box parameters
gaze_box_width_ang    = [19 19];%gaze box width. [x_width y_width] in degrees]
...
gaze_box_roll         = 15;%roll in absolute degrees from zero that will allow the trial to go ahead
...
nogo_hold_time      = 400;%delay in ms before a nogo gaze is counted as a nogo response
...
%%
%%Trial parameters
num_trials_per_stim = 300; %number of trials per stimulus position (response trials do not count toward this total)
...
%Sampling parameters
X_values =[ -40 40]; %x values to be sampled. 0 is center.
Y_values = [-40 40 ]; %y values to be sampled. 0 is center.
